...
// Wraps miniaudio's high-level ma_engine + ma_sound for the Sync Preview
// pane. One AudioPlayer plays one file at a time at its native rate / channel
// count (separate from the 16-kHz-mono decoder we hand to whisper).
class AudioPlayer {
public:
    AudioPlayer();
    ~AudioPlayer();
